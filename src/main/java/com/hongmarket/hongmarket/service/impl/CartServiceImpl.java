...
import com.hongmarket.hongmarket.enums.ProductStatusEnum;
...
public class CartServiceImpl implements ICartService {
    ...
    public ResponseVo<CartVo> add(Integer uid, CartAddForm form) {
        ...
        //judge if the product exists
        if(product == null){
            return ResponseVo.error(ResponseEnum.PRODUCT_NOT_EXIST);
        }

        //judge if it is deleted
        if(!product.getStatus().equals(ProductStatusEnum.ON_SALE.getCode())){
           return ResponseVo.error(ResponseEnum.PRODUCT_OFF_SALE_OR_DELETE);
        }

        //judge if it is out of stock
        ...
        if(StringUtils.isEmpty(value)){
            //No product in Redis, then add product
            cart = new Cart(product.getId(), quantity, form.getSelected());
        }else {
            //It does exist, then quantity + 1
    ...
    public ResponseVo<CartVo> update(Integer uid, Integer productId, CartUpdateForm form) {
        ...
        if(StringUtils.isEmpty(value)){
            //No product in Redis, then error
            return ResponseVo.error(ResponseEnum.CART_PRODUCT_NOT_EXIST);
        }
        //It does exist, then modify it
